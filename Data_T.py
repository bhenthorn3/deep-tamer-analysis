...
for i in subs:
	i_str = str(i)
	file_name = ("run_.-tag-Human_feedback-sub" + i_str + "T.csv")

	print('This is subject {0}s Tetris data:'.format(i_str) )
	...
	ratios.append(num)
	avgT = sum(ratios)/len(ratios)

xT = max(array_lengths) - 1
...
dataP_T = reshape(dataP, shape)
dataN_T = reshape(dataN, shape)
