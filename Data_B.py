...
for i in subs:
	i_str = str(i)
	file_name = ("run_.-tag-Human_feedback-sub" + i_str + "B.csv")

	print('This is subject {0}s Bowling data:'.format(i_str) )
	...
	ratios.append(num)
	avgB = sum(ratios)/len(ratios)

xB = max(array_lengths) - 1
...
dataP_B = reshape(dataP, shape)
dataN_B = reshape(dataN, shape)
