...
for i in subs:
	i_str = str(i)
	file_name = ("run_.-tag-Human_feedback-sub" + i_str + "MPM.csv")

	print('This is subject {0}s MsPacMan data:'.format(i_str) )
	...
	ratios.append(num)
	avgMPM = sum(ratios)/len(ratios)

xMPM = max(array_lengths) - 1

step_size = int(xMPM/10)
...
#	print('This is subject {0}s Vanilla Tetris data:'.format(i_str) )
	...
	for row in csv_f:
		pos_feed.append(row[2] > '0')
	del pos_feed[0]
	if len(pos_feed) < xMPM:
		extra = [False] * (xMPM - len(pos_feed))
...
#	print('This is subject {0}s Vanilla Tetris data:'.format(i_str) )
	...
	for row in csv_f:
		neg_feed.append(row[2] < '0')
	del neg_feed[0]	
	if len(neg_feed) < xMPM:
		extra = [False] * (xMPM - len(neg_feed))
...
dataP_MPM = reshape(dataP, shape)
dataN_MPM = reshape(dataN, shape)
